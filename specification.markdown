...
- Data contained within a Master Element which is not itself part of an EBML Element can trigger incorrect parsing behavior in EBML Readers.
- Extraneous copies of Identically Recurring Element, making parsing unnecessarily slow to the point of not being usable.
- Copies of Identically Recurring Element within a Parent Element that contain invalid CRC-32 Elements. EBML Readers not checking the CRC-32 might use the bogus version of the element.
- Use of Void Elements which could be used to hide content or create bogys resynchronzation points seen by some EBML Reader and not others.
